...
export function CoresTab({ books }: { books: BooksApi }) {
  ...
  const [searchOpen, setSearchOpen] = useState(true); // Always open by default now for convenience
  ...
  async function sendCoreToLore(core: Core) {
    ...
    try {
      toastInfo("Extracting lore from core...");
      const { content } = await invokeAssistant({
        data: {
          mode: "extract",
  ...
  async function sendBlockToLore(coreTitle: string, blockTitle: string, blockBody: string) {
    const text = `CORE: ${coreTitle}\nFACT: ${blockTitle} — ${blockBody}`;
    try {
  ...
  return (
    <div className="relative flex h-full flex-col bg-background">
      ...
      <div className="thin-scrollbar flex-1 overflow-y-auto px-4 py-4 pb-4">
        <div className="mx-auto max-w-2xl">
          <div className="mb-3 flex items-start justify-between gap-2">
            <div className="min-w-0">
              <h2 className="font-serif text-2xl font-semibold flex items-center gap-2">
                <Layers className="h-5.5 w-5.5 text-primary" /> Cores
              </h2>
          ...
          {searchOpen && (
            <div className="animate-slide-down-fade mb-4 flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 shadow-sm">
          ...
          {answer && (
            <div className="animate-slide-up-fade mb-4 rounded-2xl border border-border/75 bg-[color:var(--writer-bg)] p-3.5 shadow-sm">
              <div className="mb-1 flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-[color:var(--writer)]">
                  <Sparkles className="h-3.5 w-3.5 animate-pulse" /> Cores answer
                </div>
                <button
              ...
              <p className="whitespace-pre-wrap text-[13px] leading-relaxed text-foreground">{answer.text}</p>
              {answer.sources && (
          ...
          {active.cores.length === 0 && (
            <div className="rounded-2xl border border-dashed border-border p-8 text-center text-[13px] text-muted-foreground bg-card/20">
              No cores yet. Use the <span className="font-semibold text-primary">+</span> icon next to the send button in the composer below.
            </div>
          )}
            ...
            {filteredCores.map((core) => {
              ...
              if (namesOnly) {
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-xl border border-border/70 bg-card px-3 py-2 text-left hover:border-primary/40 shadow-sm"
                  >
              ...
              return (
                <CoreCard
                  ...
                  onSendBlockToLore={(blockTitle, blockBody) => sendBlockToLore(core.title, blockTitle, blockBody)}
      ...
      {/* Ask composer — compact centered, matches BrainstormTab */}
      <form
        onSubmit={(e) => {
          e.preventDefault();
          ask();
      ...
      >
        <div className="mx-auto w-full max-w-2xl">
          <div className="flex items-center gap-1.5 rounded-2xl border border-border bg-card px-2.5 py-1.5 shadow-sm">
            <Layers className="h-4 w-4 shrink-0 text-muted-foreground ml-1" />
            <input
              value={askInput}
...
function CoreCard({
  ...
  onAttach: () => void;
  onSendToLore: () => void;
  onSendBlockToLore: (title: string, body: string) => void;
  onExpand: () => void;
}) {
  ...
  return (
    <div className="animate-slide-up-fade rounded-2xl border border-border/70 bg-card p-3.5 transition-shadow hover:shadow-md">
      <div className="flex items-center justify-between gap-2">
        <div className="flex min-w-0 items-center gap-2 flex-1">
          <button
            type="button"
            onClick={() => setCollapsed(!collapsed)}
            className="flex h-6 items-center gap-1 rounded-full bg-[color:var(--writer-bg)] px-2 text-[10.5px] font-semibold text-[color:var(--writer)] hover:opacity-80"
          >
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            className="rounded-full p-1.5 hover:bg-muted text-destructive hover:bg-destructive/10"
      ...
      {!collapsed && (
        <div className="mt-3 space-y-2.5">
          {(core.attachments?.length ?? 0) > 0 && (
            <div className="flex flex-wrap gap-1.5 border-b border-border/40 pb-2">
              {core.attachments!.map((a) => (
                <div
                  key={a.id}
                  className="group flex items-center gap-1 rounded-full bg-muted px-2 py-1 text-[11px] hover:bg-muted/70 transition-colors"
                >
                  <a
                    href={a.dataUrl}
                    download={a.name}
          ...
          {/* Sub Core Facts */}
          <div className="space-y-2 max-h-[300px] overflow-y-auto thin-scrollbar pr-0.5">
            {visibleBlocks.map((bl, bi) => (
              <div
                key={bl.id}
                className="rounded-xl border border-l-2 border-border/60 border-l-[color:var(--writer)] bg-background p-2.5 shadow-sm hover:shadow transition duration-200"
              >
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="min-w-0 flex-1 bg-transparent text-[12.5px] font-semibold outline-none text-foreground"
                  />
                  <div className="flex items-center gap-0.5 shrink-0">
                    <button
                      onClick={() => onSendBlockToLore(bl.title, bl.body)}
                      className="rounded-full p-1 hover:bg-muted text-muted-foreground hover:text-foreground"
                      aria-label="Extract Lore from Fact"
                      title="Extract lore from this fact"
                    >
                      <BookOpen className="h-3 w-3" />
                    </button>
                    <button
                      onClick={() => books.removeCoreBlock(core.id, bl.id)}
                      className="rounded-full p-1 hover:bg-muted text-destructive"
                      aria-label="Delete"
                    >
                      <X className="h-3 w-3" />
                    </button>
            ...
            {adding ? (
              <div className="space-y-2 rounded-xl border border-border bg-background p-2.5 shadow-sm">
              ...
              <button
                ...
                className="flex w-full items-center gap-1.5 rounded-xl border border-dashed border-border p-2 text-left text-[12px] text-muted-foreground hover:bg-muted/45 hover:text-foreground transition duration-150"
              >
                <Plus className="h-3.5 w-3.5 text-primary" /> Add fact
              </button>
            )}
          </div>
        </div>
