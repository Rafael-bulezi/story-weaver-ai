...
export function LoreTab({ books, highlightId }: { books: BooksApi; highlightId?: string }) {
  ...
  const tabs: { id: LoreType; label: string; icon: React.ElementType }[] = [
    ...
    { id: "faction", label: "Factions", icon: Swords },
  ...
  return (
    <div className="relative mx-auto flex h-full w-full max-w-2xl flex-col">
      ...
      <div className="thin-scrollbar flex-1 space-y-2 overflow-y-auto px-4 py-4 pb-28">
        {filtered.length === 0 && (
      ...
      <FloatingAddMenu
        ...
        options={[
          ...
          { id: "faction", label: "Faction", icon: Swords, onClick: () => setPending("faction") },
...
function LoreRow({
  ...
  const Icon = item.type === "character" ? User : item.type === "place" ? MapPin : item.type === "faction" ? Swords : Lightbulb;
  if (editing) {
    return (
      <LoreEditor
        item={item}
...
export function LoreEditor({
  ...
  const genImage = generateLoreImage;
  ...
  async function generate() {
    ...
    setGenerating(true);
    try {
      const prompt = `${type === "character" ? "Portrait of a character" : type === "place" ? "Illustration of a place" : type === "faction" ? "Symbol or crest of a faction" : "Symbolic illustration of a concept"} named ${name}. ${role ? `${role}. ` : ""}${description}. Cinematic, painterly, moody lighting.`;
            ...
            placeholder={
              type === "character"
                ...
                : type === "place"
                  ? "Kind (e.g. City)"
                  : type === "faction"
                    ? "Allegiance (e.g. Antagonist group)"
                    : "Category"
            }
            value={role}
